...
##### Gen #####
Конструктор приминающий в качестве параметра функцию-генератор.
...
В качестве парамета генератору передается handle функция, позволяющая вернуть значение из монады.
...
Hand, Gen, E, isE, seq, pseq, any, lift, plift, bnd, pbnd, allseq, pallseq.
...
Метод композиции функций возвращающих монады/потоки. Массив функций должен содержать как минимум один элемент.
Метод принимает массив функций и начальное значение для передачи в первую функцию, следующие функции будут вызваны по 
мере поступления результата от предыдущих функций.
    
    // [a -> Latte b] -> a -> Latte b
    Latte.compose([
        function(v){
            return Latte.S(function(h){
        ...
        function(v){
            return Latte.M.Pack('timestamp - ' + v);
        }
    ], Date.now());
