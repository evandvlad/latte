Библиотека для работы с асинхронным кодом на основе концепции монад (обещания), потоков (frp).
...
Монада создается с помощью конструктора Latte.M. Вторым, опциональным, параметром передается контекст для функции.
...
Вторым, опциональным, параметром можно передать контекст для этой функции.
...
текущее успешное монадическое значение и возвращающую новую монаду. Вторым, опциональным, параметром можно передать контекст для этой функции.
...
(является аналогом функции fmap класса Functor с типом: (a -> b) -> f a -> f b). Вторым, опциональным, параметром можно передать контекст для этой функции.
...
возвращащую значение, которое будет обернуто в тип E. Вторым, опциональным, параметром можно передать контекст для этой функции.
...
##### repair #####
Метод используется для перехвата значения типа E, с целью исправления, он принимает функцию, принимающую значение типа E. 
Вторым, опциональным, параметром можно передать контекст для этой функции.

    // Latte.M E a -> (E a -> Latte.M b) -> Latte.M b
    Latte.M(function(h){
        h(Latte.E('error'));
    }).repair(function(e){
        return Latte.M.Pack('repair ' + e());
    });
...
##### Gen #####
Конструктор приминающий в качестве параметра функцию-генератор. Вторым, опциональным, параметром можно передать контекст для этой функции.
...
Третьим, опциональным, параметром можно передать контекст для этой функции.
...
Третим, опциональным, параметром можно передать контекст для этой функции.
...
always, next, fail, bnd, lift, raise, repair, when, unless, pass, 
