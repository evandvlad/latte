...
Благодаря методам *Latte.fun* и *Latte.callback* имеется возможность преобразовывать асинхронные функции с каллбеками в 
потоки Latte.IStream/Latte.MStream.
...
Можно задать несколько каллбеков, если это определено в оригинальной функции, в таком случае, 
вызов любого из них будет передавать значение в поток. 
...
### Быстрая упаковка значения ###

Метод *Latte.pack* получает значение и создает на его основе неизменяемый поток *Latte.IStream*. Таким образом, можно 
быстро преобразовать обычное значение в значение потока.
