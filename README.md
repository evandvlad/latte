...
Поток Latte.SH поддерживает интерфейс (все методы объекта и все статические методы) потока Latte.S, также он
проходит проверку методом Latte.isS. Отличие этого типа потока в том, что он удерживает текущее состояние
(SH - stream hold) и в этом смысле аналогичен монаде M, только в отличие от нее, может изменять это состояние.
